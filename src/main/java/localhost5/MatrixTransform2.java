...
class TransformMapperA2 extends Mapper<LongWritable, Text, IntTriple, IntTriple>{
@Override
...
class TransformMapperB2 extends Mapper<LongWritable, Text, IntTriple, IntTriple>{
@Override
protected void map(LongWritable key, Text text, Context context)
	throws IOException, InterruptedException{
		String line = text.toString();
...
class TransformReducer2 extends Reducer<IntTriple, IntTriple, IntPair, IntVector>
implements Constant{
@Override
...
throws IOException, InterruptedException{
	...
	int index = n*2;			// B向量起始下标
	IntWritable[] array = new IntWritable[list.size() - index];			// B向量
	for(int i=index;i<list.size();i++){
		array[i - index] = new IntWritable(list.get(i));
	}
	for(int i=0;i<index;i+=2){
		context.write(new IntPair(list.get(i), list.get(i+1)), new IntVector(new ArrayWritable(IntWritable.class, array)));
	}
	}
}
...
public class MatrixTransform2 extends Configured implements Tool, Constant{
	...
	public int run(String[] args)		throws Exception{
		...
		MultipleInputs.addInputPath(job, new Path(args[0]), TextInputFormat.class, TransformMapperA2.class);
		MultipleInputs.addInputPath(job, new Path(args[1]), TextInputFormat.class, TransformMapperB2.class);
